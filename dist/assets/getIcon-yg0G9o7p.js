import{D as e}from"./database-DpgD_JVg.js";import{R as t,L as r,P as i,C as a}from"./rocket-Bz9b4Ecs.js";const s={"java-intro":{id:"java-intro",course:"java",title:"Introduction to Java",videoId:"eIrMbAQSU34",content:`## What is Java?
...
- **JDK (Java Development Kit)** — JRE + compiler + developer tools`,code:`// HelloWorld.java
...
}`,language:"java",notes:["Every Java program starts execution from the main() method","Java is case-sensitive — String ≠ string","Each public class must be in a file matching its name (e.g., HelloWorld.java)","System.out.println() adds a newline; System.out.print() does not"],prev:null,next:"java-variables"},"java-variables":{id:"java-variables",course:"java",title:"Variables & Data Types",videoId:"xk4_1vDrzzo",content:`## Variables in Java
...
Java 10 introduced the \`var\` keyword for local variable type inference, letting the compiler infer the type automatically.`,code:`public class Variables {
...
}`,language:"java",notes:["Use final to declare constants; name them in UPPER_SNAKE_CASE","long literals need an L suffix: 99L","float literals need an f suffix: 3.14f","var works only for local variables, not fields or method parameters"],prev:"java-intro",next:"java-loops"},"java-loops":{id:"java-loops",course:"java",title:"Loops in Java",videoId:"P-pID6OshH4",content:`## Control Flow: Loops
...
| return    | Exit the entire method               |`,code:`public class Loops {
    public static void main(String[] args) {
...
}`,language:"java",notes:["Avoid modifying the loop variable inside the body — it reduces readability","for-each loops cannot modify the source array directly","Nested loops have O(n²) complexity — use them carefully","Label outer loops (outerLoop:) to break/continue from inner loops"],prev:"java-variables",next:"java-arrays"},"java-arrays":{id:"java-arrays",course:"java",title:"Arrays in Java",videoId:"L06uGnF4IpY",content:`## Arrays
...
Java supports 2D and higher-dimensional arrays, stored as arrays-of-arrays.`,code:`import java.util.Arrays;
...
}`,language:"java",notes:["Arrays.sort() uses dual-pivot quicksort for primitives and mergesort for objects","ArrayIndexOutOfBoundsException thrown when index ≥ length","Use ArrayList instead of arrays when you need a dynamic-size collection","Arrays.toString() gives a readable string representation"],prev:"java-loops",next:"java-methods"},"java-methods":{id:"java-methods",course:"java",title:"Methods in Java",videoId:"cCgOESMQe44",content:`## Methods
...
Java is strictly **pass-by-value**. Primitive values are copied; object references are copied (but both point to the same object).`,code:`public class Methods {
...
}`,language:"java",notes:["static methods belong to the class and can be called without an object","Avoid deep recursion — it can cause StackOverflowError","Overloading is resolved at compile time (static dispatch)","Keep methods short and focused on a single responsibility"],prev:"java-arrays",next:"java-collections"},"java-collections":{id:"java-collections",course:"java",title:"Collections Framework",videoId:"GdAon80-0KA",content:`## Java Collections Framework
...
- Need **key-value** lookups → \`HashMap\``,code:`import java.util.*;
...
}`,language:"java",notes:["Prefer List.of() and Map.of() for immutable collections","HashMap is not thread-safe — use ConcurrentHashMap in multi-threaded code","Streams do not modify the source collection","Use generics (<Type>) to avoid ClassCastException at runtime"],prev:"java-methods",next:"oop-encapsulation"},"oop-encapsulation":{id:"oop-encapsulation",course:"oop",title:"Encapsulation",videoId:"pTB0EiLXUC8",content:`## What is Encapsulation?
...
- Easier to test and maintain`,code:`public class BankAccount {
...
}`,language:"java",notes:["Make fields private by default — expose only what is necessary","Use final for fields that should never change after construction","Validate data inside setters to enforce business rules","Encapsulation is the foundation of all other OOP principles"],prev:"java-collections",next:"oop-inheritance"},"oop-inheritance":{id:"oop-inheritance",course:"oop",title:"Inheritance",videoId:"Zs342ePFvRI",content:`## What is Inheritance?
...
- Use \`final\` to prevent a class from being subclassed`,code:`// Parent class
public class Animal {
...
}`,language:"java",notes:["Use inheritance only for true IS-A relationships (Dog IS-A Animal ✓)","Prefer composition over inheritance for HAS-A relationships","Always use @Override annotation when overriding — catches typos","super() must be the first statement in a subclass constructor"],prev:"oop-encapsulation",next:"oop-polymorphism"},"oop-polymorphism":{id:"oop-polymorphism",course:"oop",title:"Polymorphism",videoId:"jhDUxynEQRI",content:`## What is Polymorphism?
...
When a parent class reference points to a child object, the JVM calls the **overridden version** of the method at runtime — this is dynamic dispatch.`,code:`// Base class
...
}`,language:"java",notes:["Runtime polymorphism is resolved by the JVM, not the compiler","You can upcast implicitly; downcast requires explicit cast + instanceof check","Use instanceof (or pattern matching in Java 16+) before downcasting","Polymorphism enables the Open/Closed Principle — open for extension, closed for modification"],prev:"oop-inheritance",next:"oop-abstraction"},"oop-abstraction":{id:"oop-abstraction",course:"oop",title:"Abstraction",videoId:"HvPlEJ3LHgE",content:`## What is Abstraction?
...
| Use when           | Shared base + some logic  | Contract definition           |`,code:`// Abstract class — partial implementation
...
}`,language:"java",notes:["Abstract classes cannot be instantiated with new","A class with even one abstract method must be declared abstract","From Java 8, interfaces can have default and static methods","From Java 9, interfaces can have private methods"],prev:"oop-polymorphism",next:"oop-interfaces"},"oop-interfaces":{id:"oop-interfaces",course:"oop",title:"Interfaces",videoId:"GhslBwrRsnw",content:"## Interfaces in Java\n\nAn interface is a **pure contract** — it defines *what* a class must do, not *how*. A class can implement multiple interfaces, solving Java's single-inheritance limitation.\n\n## Functional Interfaces (Java 8+)\n\nAn interface with exactly **one abstract method** is a functional interface. These can be used with **lambda expressions**.\n\n## Common Built-in Functional Interfaces\n\n- `Runnable` — `void run()`\n- `Comparable<T>` — `int compareTo(T)`\n- `Iterable<T>` — `Iterator<T> iterator()`\n- `Predicate<T>` — `boolean test(T)`\n- `Function<T,R>` — `R apply(T)`",code:`// Defining interfaces
@FunctionalInterface
interface Printable {
...
}`,language:"java",notes:["A class can implement multiple interfaces — this is Java's way of multiple inheritance","@FunctionalInterface annotation is optional but good practice","Default methods allow adding new methods without breaking existing implementations","Prefer interfaces over abstract classes for defining contracts"],prev:"oop-abstraction",next:"spring-ioc"},"spring-ioc":{id:"spring-ioc",course:"spring",title:"IoC Container",videoId:"aS9SQITRocc",content:`## Inversion of Control (IoC)
...
- Centralized configuration`,code:`// Service interface
...
}`,language:"java",notes:["IoC inverts the dependency graph — you declare what you need, Spring provides it","ApplicationContext is the main IoC container interface; BeanFactory is the lower-level one","Spring Boot auto-configures the ApplicationContext for you","Beans are singletons by default in Spring"],prev:"oop-interfaces",next:"spring-di"},"spring-di":{id:"spring-di",course:"spring",title:"Dependency Injection",videoId:"ET_n9On8aLQ",content:`## Dependency Injection (DI)
...
- Immediately obvious what the class needs`,code:`import org.springframework.beans.factory.annotation.Autowired;
...
}`,language:"java",notes:["Prefer constructor injection over field injection","With a single constructor, @Autowired is optional in Spring 4.3+","@Value injects values from application.properties or environment","Use @Qualifier when multiple beans of the same type exist"],prev:"spring-ioc",next:"spring-beans"},"spring-beans":{id:"spring-beans",course:"spring",title:"Spring Beans",videoId:"BARE4zjC0io",content:`## Spring Beans
...
- **session** — one per HTTP session (web apps)`,code:`import org.springframework.beans.factory.annotation.Autowired;
...
}`,language:"java",notes:["@RestController = @Controller + @ResponseBody — use for REST APIs","ResponseEntity gives full control over status codes and headers","Use @Valid with @RequestBody to trigger bean validation","DispatcherServlet is auto-configured by Spring Boot"],prev:"spring-beans",next:"sb-setup"},"sb-setup":{id:"sb-setup",course:"springboot",title:"Spring Boot Project Setup",videoId:"vtPkZShrvXQ",content:`## What is Spring Boot?
...
</dependencies>`,language:"xml",notes:["spring-boot-starter-parent manages compatible dependency versions","Each starter brings in a curated set of related dependencies","DevTools enables auto-restart and LiveReload in development","Run with: mvn spring-boot:run or java -jar target/app.jar"],prev:"spring-mvc",next:"sb-rest"},"sb-rest":{id:"sb-rest",course:"springboot",title:"Building REST APIs",videoId:"vtPkZShrvXQ",content:`## REST APIs with Spring Boot
...
}`,language:"java",notes:["Java 16+ records are perfect for immutable DTOs","@Transactional on the service layer manages database transactions","Jackson automatically serialises/deserialises Java objects to/from JSON","Always use DTOs — never expose JPA entities directly in API responses"],prev:"sb-setup",next:"sb-exceptions"},"sb-exceptions":{id:"sb-exceptions",course:"springboot",title:"Exception Handling",videoId:"PzK4ZXa2Tbc",content:`## Global Exception Handling
...
}`,language:"java",notes:["@RestControllerAdvice = @ControllerAdvice + @ResponseBody","More specific exception handlers take precedence over general ones","Never expose stack traces in production error responses","Log exceptions server-side even when returning clean error responses"],prev:"sb-rest",next:"jpa-entities"},"jpa-entities":{id:"jpa-entities",course:"jpa",title:"JPA Entities",videoId:"L06uGnF4IpY",content:`## JPA & Hibernate
...
}`,language:"java",notes:["Every entity must have a no-arg constructor (can be protected)","@Enumerated(EnumType.STRING) is safer than ORDINAL (survives enum reordering)","Use @PrePersist / @PreUpdate for auditing timestamps automatically","@Column(updatable=false) on createdAt prevents accidental overwrite"],prev:"sb-exceptions",next:"jpa-repositories"},"jpa-repositories":{id:"jpa-repositories",course:"jpa",title:"Spring Data Repositories",videoId:"TUOwAMRklag",content:"## Spring Data JPA Repositories\n\nSpring Data JPA eliminates boilerplate DAO code. You define an interface, Spring generates the implementation automatically.\n\n## Repository Hierarchy\n\n```\nRepository\n  └── CrudRepository       (basic CRUD)\n        └── PagingAndSortingRepository\n              └── JpaRepository  (most feature-rich — use this)\n```\n\n## Query Methods\n\nSpring Data derives queries from method names:\n- `findByName` → `SELECT * FROM ... WHERE name = ?`\n- `findByAgeGreaterThan` → `... WHERE age > ?`\n- `findByNameAndEmail` → `... WHERE name = ? AND email = ?`",code:`import org.springframework.data.domain.*;
...
}`,language:"java",notes:["JpaRepository provides findAll, findById, save, delete and more out of the box","Use @Transactional(readOnly=true) on the service for read operations — improves performance","@Modifying is required for @Query that UPDATE or DELETE","Pagination prevents loading millions of rows — always paginate list endpoints"],prev:"jpa-entities",next:"jpa-relationships"},"jpa-relationships":{id:"jpa-relationships",course:"jpa",title:"JPA Relationships",videoId:"C9AhBRqaVkA",content:`## Entity Relationships
...
- \`CascadeType.ALL\` — all operations cascade`,code:`import jakarta.persistence.*;
import java.util.*;
...
}`,language:"java",notes:["Always use FetchType.LAZY for @OneToMany and @ManyToMany","Use @EntityGraph or JOIN FETCH in JPQL to avoid N+1 queries","Avoid CascadeType.ALL on @ManyToOne — it can delete shared entities",'The "owning side" (with @JoinColumn) controls the foreign key in the database'],prev:"jpa-repositories",next:"jpa-jpql"},"jpa-jpql":{id:"jpa-jpql",course:"jpa",title:"JPQL Queries",videoId:"siEf9HGQaZ0",content:`## JPQL (Jakarta Persistence Query Language)
...
The **Criteria API** provides a type-safe, programmatic way to build dynamic queries — no string manipulation needed. Best used with **JPA Metamodel** for full type safety.`,code:`import jakarta.persistence.*;
import org.springframework.data.jpa.repository.*;
import org.springframework.data.repository.query.Param;
import java.util.*;
...
}`,language:"java",notes:["Always use JOIN FETCH for collections you know you'll need — avoids N+1","DTO projections (new com.example.Dto(...)) are more efficient than loading full entities","Use Criteria API for complex dynamic queries; JPQL for static ones","Named queries (@NamedQuery) are validated at startup — fail fast on typos"],prev:"jpa-relationships",next:null}},l=[{id:"java",label:"Java Basics",color:"#FF6B3D",iconName:"Coffee",items:[{id:"java-intro",label:"Introduction"},{id:"java-variables",label:"Variables & Data Types"},{id:"java-loops",label:"Loops"},{id:"java-arrays",label:"Arrays"},{id:"java-methods",label:"Methods"},{id:"java-collections",label:"Collections"}]},{id:"oop",label:"OOP Concepts",color:"#9C7CF6",iconName:"Puzzle",items:[{id:"oop-encapsulation",label:"Encapsulation"},{id:"oop-inheritance",label:"Inheritance"},{id:"oop-polymorphism",label:"Polymorphism"},{id:"oop-abstraction",label:"Abstraction"},{id:"oop-interfaces",label:"Interfaces"}]},{id:"spring",label:"Spring Framework",color:"#4CAF89",iconName:"Leaf",items:[{id:"spring-ioc",label:"IoC Container"},{id:"spring-di",label:"Dependency Injection"},{id:"spring-beans",label:"Beans"},{id:"spring-mvc",label:"Spring MVC"}]},{id:"springboot",label:"Spring Boot",color:"#F4C542",iconName:"Rocket",items:[{id:"sb-setup",label:"Project Setup"},{id:"sb-rest",label:"REST APIs"},{id:"sb-exceptions",label:"Exception Handling"}]},{id:"jpa",label:"JPA / Hibernate",color:"#E05FA0",iconName:"Database",items:[{id:"jpa-entities",label:"Entities"},{id:"jpa-repositories",label:"Repositories"},{id:"jpa-relationships",label:"Relationships"},{id:"jpa-jpql",label:"JPQL"}]}],c={Coffee:a,Puzzle:i,Leaf:r,Rocket:t,Database:e};export{c as I,s as L,l as S};
